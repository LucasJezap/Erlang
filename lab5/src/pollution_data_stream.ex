# Niestety nie znalazłem w mojej implementacji żadnego zastosowania dla Stream.filter.
# Zastosowanie protokołu Stream zamiast protokołu Enum nie dało w moim przypadku znaczących
# różnic, być może wynika to z implementacji modułu pollution. Wnioskiem z laboratorium jest
# fakt, iż język Elixir wydaje mi się bardziej przyjemny w użyciu niż język Erlang.
defmodule PollutionDataStream do
  ...
  defp parseOneLine(line) do
    ...
    dateDay = day |> String.split("-")
              |> Enum.reverse
              |> Stream.map(fn (el) -> elem(Integer.parse(el),0) end)
    ...
    location = [lat,long] |> Stream.map(fn (el) -> elem(Float.parse(el),0) end)
               |> Enum.reduce({}, fn(element,tuple) -> Tuple.append(tuple,element) end)
    %{
  ...
  defp identifyStations(parsedLines) do
    parsedLines |> Stream.flat_map(fn map -> map.location end)
    |> Stream.uniq_by(fn x -> x end)
  end
  ...
  def loadStats(parsedLines) do
    identifyStations(parsedLines) |> Stream.flat_map(fn loc -> loadStation(loc) end)
  end
  ...
  def loadVal(parsedLines) do
    parsedLines |> Stream.flat_map(fn line -> loadValue(line) end)
  end
